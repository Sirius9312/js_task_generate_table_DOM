...
"use strict";
console.log((0, _peopleJson.people).map((item)=>{
    const newObj = JSON.parse(item);
    return newObj;
})); //console.log(fetch('http://localhost:8080/lib/people.json').then((response) => JSON.parse(response).map(r => r.json())));
...
people.map((item) => {
  ...
  const newItem = {
    ...
    century: Math.ceil(item.born / 100),
  };

  for (const key in newItem) {
    const row = document.createElement('td');

    row.textContent = newItem[key];
    column.append(row);
  }
...
},{"./lib/people.json":"f4QOI"}],"f4QOI":[function(require,module,exports) {
...
},{}]},["l1XHo","3cYfC"], "3cYfC", "parcelRequireee03")
